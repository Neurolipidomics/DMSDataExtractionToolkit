...
Updated on Mon Aug 12 11:09:20 2026
...
root = tk.Tk()
root.withdraw()
messagebox.showinfo("DMS data", "In the next window, please browse to the folder where DMS files are stored")
...
root = tk.Tk()
root.withdraw()
messagebox.showinfo("DataInfo.csv", "In the next window, please upload 'DataInfo.csv'")
        ...
        if not dirnames.startswith("."):
            ...
            for filename in os.listdir(subfolderpath):
                if filename.endswith(".mzML") and "SV" in filename:
                    ...
                    df_temp['SV'] = SV_list
                    df_temp['chainLength'] = int(chainLength)
                    df_temp['unsaturationUnit'] = int(unsatUnit)
...
root = tk.Tk()
root.withdraw()
messagebox.showinfo("Save", "In the next window, please choose a location and a name to save output as.")
